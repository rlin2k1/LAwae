...
void printDirectionsRaw(GeoCoord start, float end, vector<NavSegment>& navSegments);
void printDirections(GeoCoord start, float end, vector<NavSegment>& navSegments);

vector<tuple<double, double> > returnLatLong(GeoCoord start, float end, vector<NavSegment>& navSegments); //New Implementation
...
int main(int argc, char *argv[])
{
    ...
    if ( ! nav.loadMapData("mapdata.txt") )
    {
    ...
    string startLatitude = argv[1];
    string startLongitude = argv[2];
    
    GeoCoord input = GeoCoord(startLatitude, startLongitude);
    
    
    float end = stof(argv[3]);
    vector<NavSegment> navSegments;
    
    NavResult result = nav.navigate(input, end, navSegments);
    if ( ! raw)
        cout << endl;
...
//    {
//        case NAV_NO_ROUTE:
//            cout << "No route found between " << input.latitudeText << " and " << end << endl;
//            break;
//        case NAV_BAD_SOURCE:
//            cout << "Start attraction not found: " << input.latitudeText << endl;
//            break;
...
//        case NAV_SUCCESS:
//            if (raw)
//                printDirectionsRaw(input, end, navSegments);
//            else
//                printDirections(input, end, navSegments);
//            break;
//    }
//
//    cout << "NEW SECTION IMPLEMENTATION" << endl;
    vector<tuple<double, double>> latLong = returnLatLong(input,  end, navSegments);
    
//    cout.setf(ios::fixed);
//    cout.precision(9);
    ofstream myfile;
    myfile.open ("output.txt");
    for ( const auto& i : latLong ) {
        myfile << get<0>(i) << "," << get<1>(i) << endl;
    }
    return 0;
}


vector<tuple<double, double>> returnLatLong(GeoCoord start, float end, vector<NavSegment>& navSegments) {
    vector<tuple<double, double>> latLong;
        ...
        switch (ns.m_command)
        {
            ...
            case NavSegment::TURN:
                if (distSinceLastTurn > 0)
                {
                    ...
                    tempTuple = make_tuple(effectiveSegment.start.latitude, effectiveSegment.start.longitude);
                    latLong.push_back(tempTuple);
    ...
    tuple<double, double> tempTuple;
    tempTuple = make_tuple(effectiveSegment.end.latitude, effectiveSegment.end.longitude);
    latLong.push_back(tempTuple);
...
void printDirectionsRaw(GeoCoord start, float end, vector<NavSegment>& navSegments)
{
    cout << "Start: " << start.latitudeText << endl;
    cout << "End:   " << end << endl;
    cout.setf(ios::fixed);
    cout.precision(4);
    for (auto ns : navSegments)
...
void printDirections(GeoCoord start, float end, vector<NavSegment>& navSegments)
{
    ...
    cout << "You are starting at: " << start.latitudeText << endl;
        ...
        switch (ns.m_command)
        {
            ...
            case NavSegment::TURN:
                if (distSinceLastTurn > 0)
                {
                    ...
                    cout << "Starting at Latitude, Longitude: " << effectiveSegment.start.latitude << ", " << effectiveSegment.start.longitude << endl;
                    cout << "Ending at Latitude, Longitude: " << effectiveSegment.end.latitude << ", " << effectiveSegment.end.latitude << endl;
                    thisStreet.clear();
                    distSinceLastTurn = 0;
                }
